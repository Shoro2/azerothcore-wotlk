...
void WorldObject::ModSpellCastTime(SpellInfo const* spellInfo, int32& castTime, Spell* spell) const
{
    ...
    // CoA core port: class cast-time rules (moved here from Unit::ModSpellCastTime with upstream's caster refactor)
    if (Unit const* coaCaster = ToUnit())
    {
        if (coaCaster->IsPlayer() && coaCaster->getClass() == CLASS_NECROMANCER && spellInfo->Id == 803741 &&
            coaCaster->GetPower(POWER_RUNIC_POWER) > 500)
        {
            castTime = 0;
            return;
        }
        if (coaCaster->CanCastDuringChannel(spellInfo))
        {
            castTime = 0;
            return;
        }
        if (coaCaster->getClass() == CLASS_WITCH_DOCTOR && spellInfo->SpellFamilyName == 19 &&
            (spellInfo->SpellFamilyFlags[0] & 33554432))
            castTime = int32(castTime / (1.0f + std::max(0.0f, coaCaster->GetStat(STAT_AGILITY)) / 1000.0f));
    }
...
SpellMissInfo WorldObject::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    // CoA core port: a Starcaller under 802203 cannot miss (moved here from Unit::MagicSpellHitResult)
    if (unitCaster && unitCaster->IsPlayer() && unitCaster->getClass() == CLASS_STARCALLER && unitCaster->HasAura(802203))
        tmp = 0;
...
SpellMissInfo WorldObject::SpellHitResult(Unit* victim, Spell const* spell, bool canReflect)
{
    ...
    switch (spellInfo->DmgClass)
    {
        case SPELL_DAMAGE_CLASS_RANGED:
        case SPELL_DAMAGE_CLASS_MELEE:
            // CoA core port: the spell's script may pick the off-hand for the melee roll
            if (Unit* unitCaster = ToUnit())
                return unitCaster->MeleeSpellHitResult(victim, spellInfo, spell->GetScriptMeleeAttackType());
