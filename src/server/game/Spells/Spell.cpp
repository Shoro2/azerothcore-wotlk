...
SpellCastResult Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if (!HasTriggeredCastFlag(TRIGGERED_IGNORE_CAST_IN_PROGRESS) && unitCaster &&
        !unitCaster->CanCastDuringChannel(m_spellInfo) &&
        unitCaster->IsNonMeleeSpellCast(false, true, true, m_spellInfo->Id == 75) && m_cast_count)
    {
    ...
    if ((m_spellInfo->IsChanneled() || m_casttime) && unitCaster && unitCaster->IsPlayer() && unitCaster->isMoving() &&
    ...
    else
    {
        ...
        if (unitCaster && !HasTriggeredCastFlag(TRIGGERED_IGNORE_AURA_INTERRUPT_FLAGS) && m_spellInfo->IsBreakingStealth())
        {
            ...
            unitCaster->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_CAST, exceptSpellId, m_spellInfo->Id == 75, m_spellInfo);
            unitCaster->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_SPELL_ATTACK, exceptSpellId, m_spellInfo->Id == 75, m_spellInfo);
        }
...
void Spell::update(uint32 difftime)
{
    ...
    if ((unitCaster && unitCaster->IsPlayer() && m_timer != 0) &&
            unitCaster->isMoving() && !unitCaster->HasManastormMovementGrace() &&
    ...
    if (unitCaster && unitCaster->IsPlayer())
    {
        ...
        if (pItem)
        {
            ...
            else
            {
                ...
                else if (unitCaster->HasAura(46699) || (IsAscensionClass(unitCaster->getClass()) &&
...
void Spell::TakePower()
{
    ...
    {
        int32 spent = unitCaster->ModifyHealth(-(int32)m_powerCost);
        if (spent < 0 && unitCaster->IsPlayer() && unitCaster->getClass() == CLASS_SON_OF_ARUGAL &&
            m_spellInfo->SpellFamilyName == 26 && !IsTriggered() &&
            unitCaster->HasAura(AscensionBloodmage::PooledVitalityTalent))
...
void Spell::TakeAmmo()
{
    ...
    if (unitCaster->IsPlayer() && IsAscensionClass(unitCaster->getClass()))
        return;
...
SpellCastResult Spell::CheckCast(bool strict, uint32* /*param1*/, uint32* /*param2*/)
{
    ...
    Aura const* divineCharge = unitCaster ? unitCaster->GetAura(527272) : nullptr;
    ...
    bool divineSteed = divineCaster && divineCaster->IsPlayer() && divineCaster->getClass() == CLASS_MONK &&
        unitCaster->GetMountID() == 14584 && !unitCaster->IsInFlight();
    bool tinkerMechsuit = unitCaster && unitCaster->IsPlayer() && unitCaster->getClass() == CLASS_TINKER &&
        m_spellInfo->SpellFamilyName == 34 && !unitCaster->IsInFlight() &&
        unitCaster->HasAura(801384, unitCaster->GetGUID()) && unitCaster->HasAura(803451, unitCaster->GetGUID());
    if (unitCaster && unitCaster->IsMounted() && unitCaster->IsPlayer() && !HasTriggeredCastFlag(TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE) &&
            ...
            !tinkerMechsuit &&
            !(unitCaster->getClass() == CLASS_STARCALLER && unitCaster->HasAura(704772) && !unitCaster->IsInFlight()))
    {
...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    if (target)
    {
        ...
        if (unitCaster && range_type == SPELL_RANGE_RANGED && !unitCaster->IgnoresSpellMinRange(m_spellInfo))
        {
...
void Spell::TriggerGlobalCooldown()
{
    ...
    if (unitCaster->GetCharmInfo())
        ...
        m_globalCooldownGeneration = unitCaster->ToPlayer()->GetGlobalCooldownMgr().AddGlobalCooldown(m_spellInfo, gcd);
}
...
void Spell::CancelGlobalCooldown()
{
    ...
    // Only players or controlled units have global cooldown
    if (unitCaster->GetCharmInfo())
        unitCaster->GetCharmInfo()->GetGlobalCooldownMgr().CancelGlobalCooldown(m_spellInfo, m_globalCooldownGeneration);
    else if (unitCaster->IsPlayer())
        unitCaster->ToPlayer()->GetGlobalCooldownMgr().CancelGlobalCooldown(m_spellInfo, m_globalCooldownGeneration);
}
